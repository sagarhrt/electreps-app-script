			...
			//to access excel file 
		      ...
		      //for loop for run all row testcases
		        for (int row=1; row<=rowCount; row++)
		        {
		            ...
		            String notify= sheet.getRow(row).getCell(7).getStringCellValue();
		            String youtube= sheet.getRow(row).getCell(8).getStringCellValue();
	...
	//youtube link					 
						 ...
						 if(youtube.equals(s77)){
							 ...
							 WebElement tumbnail = driver.findElement(By.xpath("//*[@id=\"edit-field-you-tube-video-link-und-0-input\"]"));
							   //click on ‘Choose file’ to upload the desired file
							   tumbnail.sendKeys(sheet.getRow(row).getCell(8).getStringCellValue()); //Uploading the video file using sendKeys
